...
func (c *Client) UpdateIncident(pageID, incidentID string, input *UpdateIncidentInput) (*UpdateIncidentOutput, error) {
	if pageID == "" {
	...
	if err != nil {
		return nil, fmt.Errorf("error in marshalling payload: %v", err)
	}
	...
	response := new(UpdateIncidentOutput)
